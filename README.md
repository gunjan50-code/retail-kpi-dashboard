...
![Retail KPI Dashboard](docs/dashboard.png)
...
## About the data
The data is **synthetic** — generated by `src/generate_raw_data.py`, which
creates realistic retail transactions and deliberately injects real-world
data-quality issues (missing customer IDs, duplicate rows, inconsistent country
spellings, "N/A" values, and returns). Its structure mirrors the well-known
"Online Retail" dataset, so the pipeline can be pointed at real data with minimal
changes. Generating the data this way guarantees the messy cases are present, so
the cleaning and modeling workflow can be demonstrated end-to-end.
